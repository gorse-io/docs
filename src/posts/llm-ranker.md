...
This evaluation uses the GitHub playground dataset from [GitRec](https://gitrec.gorse.io/). The dataset contains GitHub users, repositories, and interactions between users and repositories. To import the dataset, follow the [Gorse-in-one playground instructions](https://gorse.io/docs/quick-start.html#option-1-setup-gorse-in-one-node-via-playground) in the Quick Start guide.
