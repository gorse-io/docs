...
本文使用来自 [GitRec](https://gitrec.gorse.io/) 的 GitHub playground 数据集。该数据集包含 GitHub 用户、GitHub 仓库以及用户和仓库之间的交互记录。每个仓库的描述存储在 `item.Comment` 中，并作为文本嵌入模型的输入。请按照[快速上手](../docs/quick-start.md#选项-1-通过-playground-搭建-gorse-in-one-节点)导入该数据集。
