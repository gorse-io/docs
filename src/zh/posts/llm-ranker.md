...
本次评估使用来自 [GitRec](https://gitrec.gorse.io/) 的 GitHub playground 数据集。该数据集包含 GitHub 用户、GitHub 仓库以及用户和仓库之间的交互记录。请按照[快速上手](https://gorse.io/docs/quick-start.html#option-1-setup-gorse-in-one-node-via-playground)导入该数据集。
